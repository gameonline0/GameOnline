...
<div class="wrap">
  <div class="card">
    <div class="brand">
      <div class="brand-img"><img src="data:image/png;base64,__LOGO_B64__" alt="Game Online"></div>
      <div><div class="brand-name">Game Online</div><div class="brand-sub">v9.5</div></div>
    </div>
    <h1>ورود به پنل</h1>
    ...
    <div class="hint">
      ...
      <span class="hint-val" onclick="document.getElementById('pw').value='GameOnline';document.getElementById('pw').focus()">X4GKING</span>
    </div>
    <form id="form">
      <div class="field">
...
<div class="mob-top">
  <div class="ml">
    <div class="mob-logo"><img src="data:image/png;base64,__LOGO_B64__" alt="Game Online"></div>
    <span class="mob-title">Game Online</span>
  </div>
  <div class="mob-right">
...
<aside class="sidebar" id="sb">
  ...
  <div class="logo">
    <div class="logo-img"><img src="data:image/png;base64,__LOGO_B64__" alt="Game Online"></div>
    <div><div class="logo-name">Game Online</div><div class="logo-sub">v9.5</div></div>
  </div>
  <div class="nav-wrap">
...
<section class="pg on" id="pg-overview">
  ...
  <div class="dash-footer">
    <span class="df-text">Game Online v9.5 · Railway</span>
...
<section class="pg" id="pg-support">
  ...
  <div class="srv-panel">
    <div class="srv-hero">
      ...
      <div class="srv-hero-text">
        <div class="srv-hero-domain">پشتیبانی Game Online</div>
        <div class="srv-hero-sub"><span class="dot dg pulse"></span> راه‌های ارتباطی با تیم Game Online</div>
      </div>
    </div>
    <div class="srv-tiles">
      <a class="srv-tile" href="https://t.me/V2rayTun0" target="_blank" style="text-decoration:none;cursor:pointer">
        ...
        <div class="srv-tile-text"><div class="srv-tile-label">یوتیوب</div><div class="srv-tile-val">t.me/V2rayTun0</div></div>
      </a>
      ...
      <a class="srv-tile" href="https://github.com/Mehtif" target="_blank" style="text-decoration:none;cursor:pointer">
        <div class="srv-tile-icon"><i class="ti ti-brand-github"></i></div>
        <div class="srv-tile-text"><div class="srv-tile-label">گیت‌هاب</div><div class="srv-tile-val">github.com/Mehtif · @Mehtif</div></div>
      </a>
    </div>
  </div>
</section>
...
<div class="wrap">
  <div class="top">
    <div class="brand">
      <div class="brand-img"><img src="data:image/png;base64,{LOGO_B64}" alt="Game Online"></div>
      <div><div class="brand-name">Game Online</div><div class="brand-sub">v9.5</div></div>
    </div>
  ...
  <div class="footer">پشتیبانی: <a href="https://t.me/V2rayTun0" target="_blank">@V2rayTun0</a> · Game Online v9.5</div>
</div>
<script>
...
CLOUDFLARE_SCANNER_HTML = r"""<!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Game Online — Cloudflare IP Scanner</title>
<style>
body{margin:0;background:#090c14;color:#f5f7ff;font-family:Tahoma,Arial,sans-serif}.wrap{max-width:1100px;margin:auto;padding:28px}.head{display:flex;justify-content:space-between;align-items:center;margin-bottom:22px}.brand{font-size:25px;font-weight:900}.sub{color:#98a2b8;font-size:13px;margin-top:5px}.card{background:linear-gradient(145deg,#111725,#171e2e);border:1px solid rgba(255,255,255,.08);border-radius:18px;padding:20px;margin-bottom:18px}.row{display:grid;grid-template-columns:1fr 140px 140px;gap:12px}label{display:block;color:#98a2b8;font-size:12px;margin-bottom:7px}select,input{width:100%;padding:11px;border-radius:10px;border:1px solid rgba(255,255,255,.08);background:#0c111c;color:#fff}button{border:0;border-radius:10px;padding:11px 16px;background:linear-gradient(135deg,#6d5dfc,#8b7cff);color:#fff;font-weight:800;cursor:pointer}button:disabled{opacity:.5}table{width:100%;border-collapse:separate;border-spacing:0;border:1px solid rgba(255,255,255,.08);border-radius:14px;overflow:hidden}th,td{padding:13px;border-bottom:1px solid rgba(255,255,255,.08);text-align:right}th{color:#98a2b8;font-size:12px}.mono{font-family:monospace;direction:ltr;text-align:left}.ok{color:#34d399}.empty{text-align:center;color:#98a2b8;padding:30px}@media(max-width:700px){.row{grid-template-columns:1fr}.head{display:block}}
</style></head><body><div class="wrap"><div class="head"><div><div class="brand">Game Online · Cloudflare IP Scanner</div><div class="sub">اسکن محدود IPهای رسمی Cloudflare و مرتب‌سازی بر اساس Latency</div></div><button onclick="location.href='/dashboard'">بازگشت</button></div>
<div class="card"><div class="row"><div><label>رنج Cloudflare</label><select id="cidr"></select></div><div><label>پورت TCP</label><input id="port" type="number" value="443" min="1" max="65535"></div><div><label>تعداد نتیجه</label><input id="limit" type="number" value="10" min="1" max="50"></div></div><div style="margin-top:14px"><button id="scan" onclick="scan()">شروع اسکن</button></div><div id="status" style="margin-top:12px;color:#98a2b8">آماده</div></div>
<div class="card"><table><thead><tr><th>IP</th><th>وضعیت</th><th>Latency</th><th>عملیات</th></tr></thead><tbody id="results"><tr><td colspan="4" class="empty">هنوز اسکن انجام نشده است.</td></tr></tbody></table></div></div>
<script>
async function init(){let d=await (await fetch('/api/cloudflare-scanner/ranges')).json(),s=document.getElementById('cidr');d.ipv4.forEach(x=>{let o=document.createElement('option');o.value=x;o.textContent=x;s.appendChild(o)})}
async function scan(){let b=document.getElementById('scan');b.disabled=true;document.getElementById('status').textContent='در حال اسکن...';try{let q=new URLSearchParams({cidr:cidr.value,port:port.value,limit:limit.value}),r=await fetch('/api/cloudflare-scanner/scan?'+q),d=await r.json();if(!r.ok)throw Error(d.detail||'خطا');let t=document.getElementById('results');t.innerHTML=d.results.length?'':'<tr><td colspan="4" class="empty">IP قابل دسترسی پیدا نشد.</td></tr>';d.results.forEach(x=>{let tr=document.createElement('tr');tr.innerHTML='<td class="mono">'+x.ip+'</td><td class="ok">قابل دسترسی</td><td>'+x.latency_ms+' ms</td><td><button onclick="navigator.clipboard.writeText(\\''+x.ip+'\\')">کپی IP</button></td>';t.appendChild(tr)});document.getElementById('status').textContent=d.results.length+' نتیجه'}catch(e){document.getElementById('status').textContent=e.message}finally{b.disabled=false}}
init();
</script></body></html>"""
